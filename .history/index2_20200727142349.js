    ...
    for (var i = 3; i < length; i++) { //excel文件里的表格一般有标题所以不一定从0开始
        ...
        if (row && row.length > 0) {
            ...
            function  test(ms,url){
                let _request=request.get(`${url}`).then(response=>response).catch(err=>err)
            ...
            let response = await test(0.1,row[3]).catch(err=>{
                console.log('请求失败');
            }); 
            ...
            if (url && url !== "链接") {
                ...
                if (response.statusCode === 200) {
                    let response2 = await test(0.1,`https://b2b.baidu.com/inquiry/clueajax?ajax=1&wise=1&web=1&token=${token}`).catch(err=>{
                        console.log('请求失败');
                    }); 
