...
sheets.forEach(async function (sheet) {
    ...
    for (var i = 3; i < length; i++) { //excel文件里的表格一般有标题所以不一定从0开始
        ...
        if (row && row.length > 0) {
            ...
            await request({
                ...
                let contact = body && JSON.parse(body) && JSON.parse(body).data && JSON.parse(body).data.contact;
                let phone = body && JSON.parse(body) && JSON.parse(body).data && JSON.parse(body).data.phone;
    ...
    newArr.splice(0,0,["时间","编号","产品","链接","token","联系人","联系电话"]);
    writeXls(newArr)
});
